...
# 2. Here’s your full new response block
...
4/24/2025 16:50:42	Daily	Yes, I have	Ignore it	3	No, they’re outdated or reactive	Financial fraud	Extremely valuable	Definitely	Privacy-first (on-device AI)	Yes, monthly subscription	$5/month	To safeguard myself from cyber crime’s and fraud.	No tools	Data and privacy leak	Yes
...
4/29/2025 22:57:34	Daily	Yes, I have	Use an antivirus or protection app	1	No, they’re outdated or reactive	Financial fraud	Extremely valuable	Probably	Real-time phishing detection, SMS/email scanning for scams, Privacy-first (on-device AI), Alerts about insecure apps or websites, All of the above	Yes, monthly subscription	$10/month				Yes
"""

# 3. Parse new data into a DataFrame
new_responses = pd.read_csv(StringIO(new_data), sep="\t")

# 4. Merge and drop any full-row duplicates
merged = pd.concat([original, new_responses], ignore_index=True).drop_duplicates()

# 5. Save final updated file
merged.to_csv("Haven_Survey_Formatted_For_Form_updated.csv", index=False)

print(f"✅ Merged complete! Final number of rows: {len(merged)}")
